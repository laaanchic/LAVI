%% LAVI walk-through and validation
...
%
% Reads PINK noise generated with LAVI-Python and saves outputs for
% comparison with LAVI-Python results.
%
%% Add the LAVI toolbox to your Matlab path

clc
scriptFolder = pwd; % Current folder of the walkthrough script file (inside examples)
...
cd ../
LAVIpath = fullfile(pwd, 'LAVI-main'); % insert here the location in which the LAVI Matlab toolbox is saved
addpath(LAVIpath);

disp(sprintf('LAVI-Matlab folder: %s', LAVIpath))
...
dataFileName = fullfile(laviPythonPath, 'data', 'data.mat'); % the path to saved data
load(dataFileName);
%
...
durs        = 300;                   % the duration (in seconds) of each simulation. Default: the duration of the original signal
...
[LAVI,cfg] = Prepare_LAVI(cfg,dat); % if validate is True, it will save the outputs to a .mat file for comparison with Python outputs
...
pythonPinkFile = fullfile(laviPythonPath, 'data', 'python_validation_pink_outputs.mat');
if isfile(pythonPinkFile)
    load(pythonPinkFile)
    sig_lim = sig_lim_py;
else
    return
...
% Save outputs for comparison with LAVI-Python results
save(fullfile(laviPythonPath, 'data', 'matlab_validation_outputs.mat'), 'LAVI', "borders2")
